                    ...
                    if (row != null && row.Count() > 0)
                    {
                        ...
                        switch (header)
                        {
                            ...
                            case "N9":
                                ...
                                {
                                    EDI315_Detail_N9 dbTempN9 = new EDI315_Detail_N9();

                                    dbTempN9.reference_id_qualifier = rowLine[1];

                                    if (rowLine.Count() > 2)
                                        dbTempN9.reference_id = rowLine[2];

                                    if (rowLine.Count() > 3)
                                        dbTempN9.free_form_description = rowLine[3];

                                    if (rowLine.Count() > 4)
                                        dbTempN9.free_form_description = rowLine[4];

                                    if (rowLine.Count() > 5)
                                        dbTempN9.date = rowLine[5];

                                    if (rowLine.Count() > 6)
                                        dbTempN9.time = rowLine[6];

                                    if (rowLine.Count() > 7)
                                        dbTempN9.time_code = rowLine[7];

                                    dbEDI315_Detail_N9List.Add(dbTempN9);
                                }
                                #endregion
                                break;
                            ...
                            case "R4":
                                if (dbEDI315 == null || dbEDI315.EDI315_idnum == 0)
                                    continue;
                                { 
                                EDI315_Detail_R4 dbTempR4 = new EDI315_Detail_R4();

                                dbTempR4.port_function_code = rowLine[1];
                                if (rowLine.Count() > 2)
                                    dbTempR4.location_qualifier = rowLine[2];

                                if (rowLine.Count() > 3)
                                    dbTempR4.location_id = rowLine[3];

                                if (rowLine.Count() > 4)
                                    dbTempR4.port_name = rowLine[4];

                                if (rowLine.Count() > 5)
                                    dbTempR4.country_code = rowLine[5];

                                if (rowLine.Count() > 6)
                                    dbTempR4.terminal_name = rowLine[6];

                                if (rowLine.Count() > 7)
                                    dbTempR4.pier_number = rowLine[7];

                                if (rowLine.Count() > 8)
                                    dbTempR4.province_code = rowLine[8];

                                dbEDI315_Detail_R4List.Add(dbTempR4);
                                }
                                break;
