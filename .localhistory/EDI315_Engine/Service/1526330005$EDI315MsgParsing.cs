            ...
            if (msg_idnum != 0)
            {
                if (util.dbConnectionCheck())
                {
                    using (DBContext context = new DBContext())
                    {
                        ...
                        List<EDI315_Detail_R4_DTM> DTMList = context.EDI315_Detail_R4_DTM.Where(x => msg_idnum == msg_idnum.Contains(x.Detail_R4_idnum) && x.EDI315_Detail_idnum == detailRow.EDI315_Detail_idnum).ToList();
